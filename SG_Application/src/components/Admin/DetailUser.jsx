...
const DetailUser = () => {
  ...
  const { employee } = location.state || {}; // Récupérer l'employee passé via location.state
  ...
  useEffect(() => {
    if (employee) {
      setUserInfo(employee); // Remplir userInfo avec les données de l'employé si elles existent
    }
  }, [employee]);
  const fieldLabels = {
    cin: "CIN",
  ...
  const handleSaveClick =async () => {
    try {
      const response = await axios.post('http://localhost:8081/api/employer/modifier', userInfo);
      console.log('Employé modifie avec succès:', response.data);
  ...
  useEffect(() => {
    const fetchLeaveHistory = async () => {
      try {
        if (userInfo.cin) {
        const response = await axios.get(`http://localhost:8081/api/demandeConge/his/employer/${userInfo.cin}`);
    ...
    <div className="min-h-screen bg-gray-100 p-8 ">
      <div className="max-w-6xl mx-auto grid grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow-lg ">
          ...
          <ul className="mt-4 space-y-3 max-h-[360px] element-scroll">
