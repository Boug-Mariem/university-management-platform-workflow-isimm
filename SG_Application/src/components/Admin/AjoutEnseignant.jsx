...
const AjoutEnseignant = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    console.log("Enseignant Added:", formData);
    try {
      // Envoi de la requête POST à l'API backend
      const response = await axios.post('http://localhost:8081/api/enseignant/create', formData);
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <span className="absolute -top-2 left-3 bg-gray-50 px-1 text-xs text-gray-500">
                ...
                <input
                  ...
                  onChange={handleChange}
                  required
                  className="w-full bg-transparent text-blue-500 focus:outline-none"
                />
              </div>
