...
//pour enseignant
const NavbarEnseignant = ({ role,enseignant}) => {
  const navigate = useNavigate();
  const [userNotifications, setUserNotifications] = useState([]);
  useEffect(() => {
    const fetchNotifications = () => {
      axios.get(`http://localhost:8081/api/notifications/${enseignant.cin}`)
        .then((response) => {
  ...
  const handleProfileClick = () => {
    navigate('/ProfileEnseignant' ,{ state: { User: enseignant } });
  };
  ...
  const handleCongeClick = () => {
    navigate('/enseignant/DemandeDeCongeEnseignant',{ state: { User: enseignant } });  
};
...
const creerNotif=async()=>{
  try{
    const response = await axios.post(`http://localhost:8081/api/notifications/creerPourAdmins`,  null,
      {
        params: {
          description: `${enseignant.nom} ${enseignant.prenom} a fait une demande d'Attestation de Travail`,
          dateDepose: today ,
        }
      });
...
const handleAttestationClick = async () => {
  try {
      const response = await axios.post('http://localhost:8081/api/attestation/creer/enseignant', enseignant);
  ...
  return (
    <div className="bg-gradient-to-r from-blue-500 to-gray-800 p-3 shadow-lg flex justify-between items-center rounded-lg relative">
      ...
      <div className="text-lg text-white">{enseignant.nom} {enseignant.prenom}</div>
