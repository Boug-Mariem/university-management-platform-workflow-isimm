...
const EnseignantDashboard = () => {
    ...
    useEffect(() => {
        ...
        if (!token) {
            ...
            try {
                ...
                setcin(decodedToken.cin); // Récupère le CIN depuis le token
            } catch (error) {
    ...
    useEffect(() => {
      ...
      useEffect(() => {
        const fetchLeaveHistory = async () => {
          try {
            if (enseignant) {
            const response = await axios.get(`http://localhost:8081/api/demandeConge/his/enseignant/${enseignant.cin}`);
      ...
      useEffect(() => {
        const apporterRequests = async () => {
          try {
            if (enseignant) {
              const response = await axios.get(`http://localhost:8081/api/demandeConge/demandesAttenteRejeter/${enseignant.cin}`);
      ...
      useEffect(() => {
        const apporterRequestsAttes = async () => {
          try {
            if (enseignant) {
              const response = await axios.get(`http://localhost:8081/api/attestation/demandesAttenteRejeter/${enseignant.cin}`);
      ...
      useEffect(() => {
        const apporterAttes = async () => {
          try {
            if (enseignant) {
              const response = await axios.get(`http://localhost:8081/api/attestation/getattesaccepter/${enseignant.cin}`);
    ...
    const handleDeleteRequestConge = async(id) => {
      ...
      try {
        ...
        //setUserRequests((prevRequests) => prevRequests.filter(request => request.contenu.id !== id));
  ...
  return (
    <div className="min-h-screen">
        ...
        {enseignant ? (
            <>
        <NavbarEnseignant role="Enseignant"  enseignant={enseignant}/>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-20 mt-20" style={{ marginTop: '60px' }}>
            ...
            <h2 className="text-lg font-bold text-blue-500 mb-4 flex items-center"> <IoDocumentTextOutline size={30} className="mr-3"/>Demandes Effectuées</h2></div>
        ...
        <div className="flex items-center justify-center space-x-4">
            ...
            <h2 className="text-lg font-bold text-blue-500 mb-4 flex items-center"><IoDownloadOutline size={30} className="mr-3"/>Documents Disponibles</h2></div>
      ...
      <div>Chargement des données de l enseignant...</div> // Message pendant le chargement
    )}
    </motion.div>
    </div>
    </div>
